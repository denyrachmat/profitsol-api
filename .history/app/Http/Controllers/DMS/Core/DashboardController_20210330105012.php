...
class DashboardController extends Controller
{
    ...
    public function newDocumentLib()
    {
        ...
        $data = DocsMaster::with('users.group')->with(['tags' => function ($f) use ($tags) {
            $f->with('tagsMaster');
        ...
        foreach (tagsMaster::get()->toArray() as $key => $value) {
            ...
            if (count($cek) > 0) {
                foreach ($cek as $keyDet => $valueDet) {
                    $hasilTaggedDocs[$value['tags_name']][$valueDet['users']['group']['division_name']][$keyDet] = $valueDet;
                }
            }
        }

        $datanya = array_values(array_filter($data, function ($f) use ($value) {
            return count($f['tags']) === 0;
        }));

        foreach ($datanya as $keyDet2 => $valueDet2) {
            $hasilTaggedDocs["UNCATEGORIES"][$valueDet2['users']['group']['division_name']][$keyDet2] = $valueDet2;
        }
