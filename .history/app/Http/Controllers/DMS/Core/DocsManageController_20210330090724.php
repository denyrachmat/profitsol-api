...
use App\Models\DMS\Core\tagsDocDet;

class DocsManageController extends Controller
{
    public function getAllTags()
    {
        return tagsMaster::get();
    }

    public function updateTags(Request $r)
    {
        $hasil = [];
        foreach ($r->tags as $key => $value) {
            $hasil[] = tagsDocDet::updateOrCreate([
                'tags_id' => $value,
                'doc_id' => $r->doc_id
            ], [
                'tags_id' => $value,
    ...
    public function storeSharedFolder(Request $req)
    {
        $hasil = [];
        foreach ($req->users as $key => $value) {
            $hasil[] = sharedMapping::updateOrCreate([
                'folder_id' => $req->folder,
                'shared_to' => $value['value']
            ], [
                'folder_id' => $req->folder,
                'shared_to' => $value['value']
            ]);
