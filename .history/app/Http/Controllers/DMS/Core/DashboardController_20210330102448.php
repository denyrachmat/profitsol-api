...
class DashboardController extends Controller
{
    ...
    public function newDocumentLib()
    {
        ...
        foreach (tagsMaster::get()->toArray() as $key => $value) {
            $hasilTaggedDocs[$value['tags_name']] = array_values(array_filter($data, function ($f) use ($value) {
                return count(array_filter($f['tags'], function ($f2) use ($value) { return $f2['tags_id'] == $value['id']; })) > 0;
            }));
        }

        $hasilTaggedDocs["UNCATEGORIES"] = array_values(array_filter(tagsMaster::get()->toArray(), function ($f) use ($value) {
