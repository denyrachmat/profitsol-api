...
class HSCodeUploadController extends BaseController
{
    ...
    {
        ExportHSCodeQueue::dispatch(
            $request->filter,
            $withHist === 'false' ? false : (bool) $withHist,
            'excel',
            $request->username,
            $downloadLast === 'false' ? false : (bool) $downloadLast
        )->onQueue('hsCodeDownloader');
        ...
        foreach (array_values($hasilGroup) as $keyGrp => $valueGrp) {
            ...
            $hasil[] = $this->approveAction(new ApprovalRunningApproveActionRequest([
                ...
                'onApproval' => [
                    ...
                    'url' => 'https://api.sumitronics-indonesia.com/api/div/log/updateApprovalHSCode'
