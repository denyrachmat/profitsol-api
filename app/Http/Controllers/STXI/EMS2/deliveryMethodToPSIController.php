...
class deliveryMethodToPSIController extends BaseController
{
    ...
    public function DLVWithBarcode(Request $req)
    {
        ...
        foreach ($req->model as $key => $value) {
            ...
            $dataCPO = DB::connection('sqlsrv_mega_tyo')->select(DB::raw("SET NOCOUNT ON; EXEC Z_STXI_GET_CPO_DLV_STXI_ITEC @model = '" . $value . "', @date_start = '" . date('Y-m-01', strtotime($req->date)) . "', @date_to = '" . $date_to . "'"))[0];
