...
class deliveryMethodToPSIController extends BaseController
{
    ...
    public function DLVWithBarcode(Request $req)
    {
        ...
        foreach ($req->model as $key => $value) {
            ...
            $dataCPO = collect(DB::connection('sqlsrv_mega_tyo')->select("EXEC Z_STXI_GET_CPO_DLV_STXI_ITEC @model = '" . $value . "', @date_start = '" . date('Y-m-01', strtotime($req->date)) . "', @date_to = '" . $date_to . "'"))[0];
