    ...
    public function searchGIT(Request $request)
    {
        ...
        if ((clone $hist)->count() > 0) {
            ...
            foreach ($datanya as $key => $value) {
                $hasil[$value['PGITSHP_SHPREFNO'] ? $value['PGITSHP_SHPREFNO'] : $value['PGIT_SUPNO']]['PGITSHP_SHPREFNO'] = $value['PGITSHP_SHPREFNO'];
                $hasil[$value['PGITSHP_SHPREFNO'] ? $value['PGITSHP_SHPREFNO'] : $value['PGIT_SUPNO']]['det'][] = array_merge(
                    $value,
                    // [
