...
class CirtenUpdateController extends BaseController
{
    ...
    public function resubmitCirten(string $id, string $username = '')
    {
        ...
        if (!empty($cirtenMstr)) {
            ...
            if (!empty($importer->data) && isset($importer->data) && isset($importer->data['send_data']) && !empty($importer->data['send_data'])) {
                ...
                return $this->handleResponse($importer->data, 'Re-sync TEN ' . $id . ' On progress');
            } else {
                Redis::publish('portalv2', json_encode([
                    'app' => 'cirten',
    ...
    public function syncTenList($year)
    {
        ...
        Excel::import($importer1, 'Technical Notice List/TECHNICAL NOTICE LIST - PRINTER IEI.xlsx', 'la_bim_ten_list');

        $importer2 = new ImportTENList($year, 0);

        Excel::import($importer2, 'Technical Notice List/TECHNICAL NOTICE LIST - PROJECTOR.xlsx', 'la_bim_ten_list');
