...
class ExportMRPSchemeWeekly implements FromCollection, WithHeadings, WithEvents
{
    ...
    public function headings(): array
    {
        ...
        foreach ($getLeadTimeList as $key => $valueLT) {
            ...
            $lastDatePerLTOriginal = (new \DateTime($this->data['first_date']))->modify('+' . ((int) ($valueLT['lt_(days)']) + 1) . ' days')->format('Y-m-d');
            ...
            if (!empty($getDateData)) {
                ...
                $this->dataPerListOriginal[(int) $valueLT['lt_(days)']] = $this->getData($this->data['first_date'], $lastDatePerLTOriginal);
            }
        }
        ...
        foreach ($this->dataPerlist as $key => $value) {
            ...
            foreach ($this->dataPerListOriginal[$key] as $keyOri => $valueOri) {
                if (isset($value[$keyOri])) {
                    $hasil[$startKeysRows][0] = date('d/m/Y', strtotime($valueOri . ' +1 day'));
                }
            }
    ...
    public function getListLT()
    {
        try {
            ...
            logger('URL Get LT List', [env('APP_URL') . '/api/mrs/runningReportFromAPI/MRSAPI_69662c0fd6adf']);
